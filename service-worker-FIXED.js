...
self.addEventListener('push', (event) => {
  ...
  catch (e) { data = { title: 'Rakesh Mart', body: event.data ? event.data.text() : 'Koi notification hai!' }; }
  ...
  const options = {
    ...
    tag: data.tag || 'rakesh-mart-' + Date.now(),
    renotify: true,
    vibrate: [200, 100, 200],
    data: { url: data.url || '/website6/', timestamp: Date.now() },
    actions: [
      { action: 'open', title: 'Dekhte Hain' },
      { action: 'close', title: 'Baad Mein' }
    ]
  };

  // Rich image support (GitHub RAW URL etc.)
...
self.addEventListener('notificationclick', (event) => {
  ...
  const urlToOpen = (event.notification.data && event.notification.data.url) ? event.notification.data.url : '/website6/';
  event.waitUntil(
    clients.matchAll({ type: 'window', includeUncontrolled: true }).then((clientList) => {
      for (const client of clientList) {
        if (client.url.includes('website6') && 'focus' in client) return client.focus();
      }
